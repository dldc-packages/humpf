...
function createInternal(
  steps: Array<SpringSequenceStep>,
  config: ISpringSequenceConfig,
): ISpringSequence {
  ...
  let initialSpring: ISpringFn = Spring({
    ...defaultConfig,
    ...resolveInitialConfig(config.initial ?? {}),
  });
  ...
  function updateFromIndex(index: number): void {
    ...
    let prev: ISpringFn = indexResolved === 0
      ? initialSpring
      : stepSpringOrThrow(steps[index - 1]);
  ...
  function insertAt(
    ...
    const newStep: SpringSequenceStep = {
      time,
  ...
  function replaceTail(
    ...
    steps.splice(stepIndex + 1, steps.length - stepIndex, {
      time,
      config: resolveConfig(config),
      spring: null,
    });
...
function resolveConfig(
  conf: number | Partial<ISpringConfig>,
): Partial<ISpringConfig> {
